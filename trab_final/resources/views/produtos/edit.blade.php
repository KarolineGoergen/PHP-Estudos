@extends('templates.main', ['titulo' => "Alterar Produto"])
@section('titulo') Produto @endsection
@section('conteudo')

<form action="{{ route('produtos.update', $dados['id']) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="row">
            <div class="col" >
            ...
            <div class="col">
                ...
                <label class="input-group-text" for="inputGroupSelect01" >Empresa</label>
                    <select name="empresa_id" class="form-control {{ $errors->has('empresa_id') ? 'is-invalid' : '' }}">
                        ...
                        <option value="{{$item->id}}" @if($item->id == $dados['empresa_id']) selected="true" @endif>
                            {{ $item->nome }}
                        </option>
                        @endforeach
                    </select>
